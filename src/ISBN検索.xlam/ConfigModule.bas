...
' --------------------------------------------------------------------------
' シート設定をブックから読み込む
' まだブックにシート設定が書き込まれていない場合はレジストリ、もしくは初期値が返される
'
' Parameter:
'    [wb]   IN ワークブック、Nothingの場合はワークブックからは読み込まない
'
' Return value:
'    シート設定
' --------------------------------------------------------------------------
...
' --------------------------------------------------------------------------
' シート設定をブックとレジストリに書き込む
'
' Parameter:
'    [wb]   IN ワークブック、Nothingの場合はワークブックには書き込まない.
'    [conf] IN シートの設定
' -------------------------------------------------------------------------
...
' キーはキー値として保存される。
' 値は、キー、値、型情報の３要素からなるArrayとして保存される.
'--------------------------------------------------------------
